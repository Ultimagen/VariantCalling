...
def filter_cnv_lcr(sample1_basename,sample2_basename,lcr_file,intersection_cutoff=0.5):
    ...
    return [count_sample1_specific_len_lcr, count_sample2_specific_len_lcr, count_sample1_common_len_lcr, count_sample2_common_len_lcr]
...
def run(argv):
    ...
    parser = argparse.ArgumentParser(
        prog="CNV_eval", description="Runs concocrdance for 2 given CNV bed files."
    )
    ...
    [df_concordance['sample1_specific'],df_concordance['sample2_specific'],df_concordance['sample1_common'],df_concordance['sample2_common']] = intersection(sample1_basename, sample2_basename, sample1_bed_file, sample2_bed_file)
    [df_concordance['sample1_specific_10K'] ,df_concordance['sample2_specific_10K'],df_concordance['sample1_common_10K'],df_concordance['sample2_common_10K']] = filter_len(sample1_basename,sample2_basename)
    [df_concordance['sample1_specific_10K_lcr'],df_concordance['sample2_specific_10K_lcr'],df_concordance['sample1_common_10K_lcr'],df_concordance['sample2_common_10K_lcr']] = filter_cnv_lcr(sample1_basename,sample2_basename,ug_cnv_lcr)

    df_concordance['percision'] = df_concordance['sample1_common_10K_lcr']/(df_concordance['sample1_common_10K_lcr']+df_concordance['sample1_specific_10K_lcr'])
    df_concordance["percision"] = pd.to_numeric(df_concordance["percision"], downcast='float')
    df_concordance['recall'] = df_concordance['sample2_common_10K_lcr']/(df_concordance['sample2_common_10K_lcr']+df_concordance['sample2_specific_10K_lcr'])
