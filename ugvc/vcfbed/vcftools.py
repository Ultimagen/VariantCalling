...
def header_record_number(header: pysam.VariantHeader) -> dict:
    """Number

    Parameters
    ----------
    header : pysam.VariantHeader
        A pysam variant header record
...
def subsample_to_alleles(field: tuple, number: str, alleles: tuple) -> tuple:
    ...
    """
    assert isinstance(field, tuple), "Scalars do not require subsampling"
    assert isinstance(number, (str, int)), "Number should be a string or an integer"
    if number == "A":
        result = tuple(([field[i] for i in j]) for j in alleles[1:])
    elif number == "R":
        result = tuple(([field[i] for i in j]) for j in alleles)
    elif number == "G":
