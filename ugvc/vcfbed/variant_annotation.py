...
import ugvc.flow_format.flowBasedRead as flowBasedRead
import ugvc.utils.misc_utils as utils
import ugvc.vcfbed.bed_writer as bed_writer
    ...
    def _is_hmer(rec, fasta_idx):
        ...
        if rec["indel_classify"] == "del":
            ...
            if len(set(del_seq)) != 1:
                ...
                fasta_idx[rec["chrom"]][rec["pos"] + len(rec["ref"]) - 1].seq.upper()
                != del_seq[0]
            ):
                return (0, None)
            else:
                return (
                    len(del_seq)
                    + dnautils.hmer_length(
                        fasta_idx[rec["chrom"]], rec["pos"] + len(rec["ref"]) - 1
                    ),
                    del_seq[0],
                )

    results = concordance.apply(
        lambda x: _is_hmer(x, fasta_idx), axis=1, result_type="reduce"
    )
...
def get_motif_around(
    concordance: pd.DataFrame, motif_size: int, fasta: str
) -> pd.DataFrame:
    ...
    def _get_motif_around_non_hmer_indel(rec, size, faidx):
        ...
        return (
            ...
            chrom[
                pos + len(rec["ref"]) - 1: pos + len(rec["ref"]) - 1 + size
            ].seq.upper(),
        )
    ...
    faidx = pyfaidx.Fasta(fasta_file, build_index=False, rebuild=False)
...
def get_gc_content(
    concordance: pd.DataFrame, window_size: int, fasta: str
) -> pd.DataFrame:
    """Extract sequence around the indel
...
def get_coverage(
    ...
    chrom2f = [
        (list(pbw.open(f).chroms().keys())[0], f)
        for f in bw_coverage_files_high_quality
    ]
    chrom2bw_dict = dict(chrom2f)

    chrom2f = [
        (list(pbw.open(f).chroms().keys())[0], f) for f in bw_coverage_files_low_quality
    ]
    for v in chrom2f:
...
def close_to_hmer_run(
    ...
    for chrom in gdf.groups.keys():
        ...
        close_dist = (
            abs(pos1 - pos2[np.clip(pos1_closest_pos2_start, 0, None)]) < max_distance
        )
        close_dist |= (
            abs(pos2[np.clip(pos1_closest_pos2_start + 1, None, len(pos2) - 1)] - pos1)
            < max_distance
        )
        pos2 = np.array(run_df.loc[grun_ix, "end"])
        pos1_closest_pos2_end = np.searchsorted(pos2, pos1)
        close_dist |= (
            abs(pos1 - pos2[np.clip(pos1_closest_pos2_end - 1, 0, None)]) < max_distance
        )
        close_dist |= (
            abs(pos2[np.clip(pos1_closest_pos2_end, None, len(pos2) - 1)] - pos1)
            < max_distance
        )
...
def fill_filter_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result = np.array(
        ["HPOL_RUN"] * fill_column_locs.sum(), dtype=f'<U{len("HPOL_RUN")}'
    )
...
def annotate_cycle_skip(
    df: pd.DataFrame, flow_order: str, gt_field: str = None
) -> pd.DataFrame:
    ...
    if gt_field is None:
        alt = np.array(
            snps["alleles"].apply(lambda x: x[1] if len(x) > 1 else None)
        ).astype(np.string_)
    else:
        ...
        alt = np.array(snps.apply(lambda x: x["alleles"][x["nra_idx"]], axis=1)).astype(
            np.string_
        )
    ...
    ref_encs = [
        utils.catch(
            flowBasedRead.generateKeyFromSequence,
...
expecting input of ref and alt sequences composed of 3 bases where only the 2nd base differs"""
    ...
    try:
        ref_key = np.trim_zeros(
            flowBasedRead.generateKeyFromSequence(ref, flow_order), "f"
        )
        alt_key = np.trim_zeros(
            flowBasedRead.generateKeyFromSequence(alt, flow_order), "f"
        )
