...
from ugvc.pipelines import evaluate_concordance, run_comparison_pipeline

ap = argparse.ArgumentParser(
    prog="run_somatic_comparison_and_graphs.py",
...
ap.add_argument("--reference_dict", help="Reference genome dictionary", required=False, type=str)
...
ap.add_argument("--is_mutect", help="Are the VCFs output of Mutect (false)", action="store_true")
...
ap.add_argument("--output_prefix", help="Output prefix", required=True, type=str)
ap.add_argument(
    "--output_suffix",
...
ap.add_argument(
    "--gs_folder",
    help="Google cloud folder to upload the results to",
    required=False,
    type=str,
)
ap.add_argument(
    "--json_db",
    help="Json file which contains all the runs that we ran, for future comparison",
    required=False,
    type=str,
)
...
if args.regions_bed is not None:
    cmp_interval = pjoin(args.output_folder, "cmp_interval_in_regions_only.bed")
    with open(cmp_interval, "w", encoding="utf-8") as outfile:
        cmd = [
            "bedtools",
            "intersect",
            "-a",
...
cmd2 = [
    ...
    "-Oz",
    "-o",
    pjoin(args.output_folder, "tmp_no_mutliallelic.vcf.gz"),
    "-e",
    f"INFO/AN>{N_ALLELES}",
    "-",
]
...
processNames = subprocess.run(cmd2, input=ps.stdout, capture_output=True, check=True)
...
cmd = [
    ...
    str(args.n_parts),
    "--input_prefix",
    pjoin(args.output_folder, "tmp_no_mutliallelic"),
    "--output_file",
    args.output_file,
    "--output_interval",
...
if args.json_db is not None:
    ...
    if not os.path.isfile(args.json_db):
        a = []
        a.append(entry)
        with open(args.json_db, mode="w", encoding="utf-8") as f:
            f.write(json.dumps(a, indent=2))
    else:
        with open(args.json_db, encoding="utf-8") as feedsjson:
