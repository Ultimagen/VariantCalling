...
class VariantHitFractionCaller:
    ...
    def calc_hit_fraction(self, called_vcf: str, ground_truth_vcf: str) -> tuple[float, float, float]:
        ...
        # remove mpileup's extra allele
        called_variants['alleles'] = called_variants.apply(lambda x: x["alleles"][:-1], axis=1)
        ground_truth_count = len(ground_truth_variants)
        hits = pd.merge(ground_truth_variants, called_variants, how="inner", on=["chrom", "pos", "ref", "alleles"])
        hit_count = len(hits)
