...
def reinterpret_variants(
    concordance_df: pd.DataFrame, input: dict, fasta: pyfaidx.Fasta
) -> pd.DataFrame:
    ...
    variant_intervals = [
        vcftools.get_region_around_variant(x, input["pos_gtr"])
    ...
    # flow to hmer indels
    concordance_df = _apply_corrections(
        concordance_df, input["fps"].index, corrections_fps
    )
    ...
    concordance_df = _apply_corrections(
        concordance_df, input["fns"].index, corrections_fns
    )
...
def compare_two_sets_of_variants(
    ...
    for i in range(len(positions_to_test)):
        ...
        flows_set2 = [
            fbr.generateKeyFromSequence(x, DEFAULT_FLOW_ORDER) for x in haplotypes_set2
        ]
        ...
        flows_set1 = [
            fbr.generateKeyFromSequence(x, DEFAULT_FLOW_ORDER) for x in haplotypes_set1
        ]
            ...
            if len(sf) != len(df):
                ...
                if (d != 0).sum() <= best_diff[0]:
                    ...
                    if best_diff[0] == 1:
                        ...
                        elif cur_best_n_changes == 1 and sf[d != 0][0] > cur_best_hmer:
                            best_diff = (1, sf[d != 0][0])
                        else:
...
def _apply_alleles_to_ref(chunks: list, combination: list) -> str:
    ...
    return "".join(
        map(
            lambda x: x[0] + x[1],
            itertools.zip_longest(chunks, combination, fillvalue=""),
        )
    )
...
def _convert_fns_to_tps(_df: pd.DataFrame) -> pd.DataFrame:
    ...
    subdf = _df.loc[
        change_annotation, ["indel", "classify", "hmer_indel_length", "qual", "sor"]
    ].copy()
    subdf["classify"] = "tp"
    subdf["qual"] = 300
    subdf["sor"] = 1
...
def _mark_hmer_indels(_df: pd.DataFrame) -> pd.DataFrame:
    ...
    _df.loc[hmer_indels, "hmer_indel_length"] = _df.loc[
        hmer_indels, "compare_to_gtr_hmer_indel_len"
    ]
    return _df


def _apply_corrections(
    _df: pd.DataFrame, positions: pd.Index, corrections: list
) -> pd.DataFrame:
    ...
    if (
        len(positions) == 1
    ):  # solves a boundary case where there is a single position and hmer_indel_to_gtr is an array
        _df.at[positions[0:1], "compare_to_gtr_hmer_indel_len"] = hmer_indel_to_gtr[0]
        _df["compare_to_gtr_hmer_indel_len"] = _df[
...
def _select_best_region(
    interval_starts: list, intervals: list, pos: int
) -> Optional[tuple]:
