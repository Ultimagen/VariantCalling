...
class ConditionalAlleleDistribution:
    ...
    def update_distribution(self, other) -> None:
        for conditioned_genotype, observed_allele_counts in other.allele_counts_dict.items():
    ...
    def get_allele_count(self, conditioned_genotype: str, observed_alleles: str, allele: str, strand: StrandDirection):
        observed_alleles_key = self.define_observed_alleles_key(observed_alleles)
    ...
    def get_observed_alleles_frequency(self, conditioned_genotype: str, observed_alleles: str) -> float:
        ...
        if observed_alleles_key in self.num_of_samples_with_alleles[conditioned_genotype] and \
                self.num_of_samples_with_alleles[conditioned_genotype][observed_alleles_key] > 0:
    ...
    def get_string_records(self, chrom: str, pos: int):
        ...
        for conditioned_genotype in self.num_of_samples_with_alleles:
            for observed_alleles, count in self.num_of_samples_with_alleles[conditioned_genotype].items():
                s = self.get_allele_counts_string(conditioned_genotype, observed_alleles)
                records.append(f'{chrom}\t{pos}\t{self.conditioned_alleles}\t{conditioned_genotype}'
