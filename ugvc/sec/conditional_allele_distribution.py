...
class ConditionalAlleleDistribution:
    ...
    def __init__(
        ...
        self.allele_counts_dict[conditioned_genotype] = {
            observed_alleles_key: allele_counts
        }
        self.num_of_samples_with_alleles[conditioned_genotype] = {
    ...
    def update_distribution(self, other) -> None:
        for (
            ...
            if conditioned_genotype not in self.num_of_samples_with_alleles:
                self.num_of_samples_with_alleles[
                    conditioned_genotype
                ] = other.num_of_samples_with_alleles[conditioned_genotype]
                self.allele_counts_dict[
                    conditioned_genotype
                ] = other.allele_counts_dict[conditioned_genotype]
            else:
                for observed_alleles, allele_counts in observed_allele_counts.items():
                    if (
                        observed_alleles
                        in self.num_of_samples_with_alleles[conditioned_genotype]
                    ):
                        self_allele_counts = self.allele_counts_dict[
                            conditioned_genotype
                        ][observed_alleles]
    ...
    def get_allele_count(
        ...
        observed_alleles_key = self.define_observed_alleles_key(observed_alleles)
        return self.allele_counts_dict[conditioned_genotype][observed_alleles_key][
            allele
        ].get_count(strand)

    def get_allele_counts(
        self, conditioned_genotype: str, observed_alleles: str, allele: str
    ) -> ReadCounts:
        observed_alleles_key = self.define_observed_alleles_key(observed_alleles)
        return self.allele_counts_dict[conditioned_genotype][observed_alleles_key][
            allele
    ...
    def get_allele_counts_string(
        self, conditioned_genotype: str, observed_alleles: str
    ):
        observed_alleles_key = self.define_observed_alleles_key(observed_alleles)
        allele_counts_fields = []
        for allele, read_counts in self.allele_counts_dict[conditioned_genotype][
            observed_alleles_key
        ].items():
            allele_counts_fields.extend([allele, read_counts.get_counts_tuple_as_str()])
        return " ".join(allele_counts_fields)

    def get_observed_alleles_frequency(
        self, conditioned_genotype: str, observed_alleles: str
    ) -> float:
        observed_alleles_key = self.define_observed_alleles_key(observed_alleles)
        total_count = sum(
            self.num_of_samples_with_alleles[conditioned_genotype].values()
        )
        if (
            observed_alleles_key
            in self.num_of_samples_with_alleles[conditioned_genotype]
            and self.num_of_samples_with_alleles[conditioned_genotype][
                observed_alleles_key
            ]
            > 0
        ):
    ...
    def get_observed_alleles_counts_list(
        self, conditioned_genotype: str, observed_alleles: str
    ) -> List[int]:
        """
        ...
        """
        observed_alleles_key = self.define_observed_alleles_key(observed_alleles)
        actual_allele_counts = self.allele_counts_dict[conditioned_genotype][
            observed_alleles_key
        ]
    ...
    def get_string_records(self, chrom: str, pos: int):
        ...
        for conditioned_genotype in self.num_of_samples_with_alleles:
            for observed_alleles, count in self.num_of_samples_with_alleles[
                conditioned_genotype
            ].items():
                s = self.get_allele_counts_string(
                    conditioned_genotype, observed_alleles
                )
                records.append(
                    f"{chrom}\t{pos}\t{self.conditioned_alleles}\t{conditioned_genotype}"
                    f"\t{observed_alleles}\t{count}\t{s}"
                )
        return records
    ...
    def define_observed_alleles_key(observed_alleles: str):
        ...
        else:  # sort alts in multi-allelic positions
...
def get_allele_counts_list(
    actual_allele_counts: Dict[str, ReadCounts], observed_alleles: str
):
    """
    ...
    for i, allele in enumerate(observed_alleles.split(",")):
        if allele in actual_allele_counts:
            actual_distribution_list.append(
                actual_allele_counts[allele].get_count(StrandDirection.FORWARD)
            )
            actual_distribution_list.append(
                actual_allele_counts[allele].get_count(StrandDirection.REVERSE)
            )
        else:
