...
from ugvc.sec.conditional_allele_distribution import (
    ConditionalAlleleDistribution,
    get_allele_counts_list,
)
...
class SECRecord:
    ...
    def __init__(
        ...
        self.actual_distribution_list = get_allele_counts_list(
            actual_allele_counts, observed_alleles
        )
        self.were_observed_alleles_in_db = (
            observed_alleles
            in expected_distribution.get_possible_observed_alleles(conditioned_genotype)
            and expected_distribution.num_of_samples_with_alleles[conditioned_genotype][
                observed_alleles
            ]
            > 0
        )
        if self.were_observed_alleles_in_db:
            ...
            if is_snp(self.observed_alleles_list):
                for i in (0, 1):
                    expected_distribution_arr[i] = (
                        1 / self.noise_ratio_for_unobserved_snps
                    )
            else:
                for i in (0, 1):
                    expected_distribution_arr[i] = (
                        1 / self.noise_ratio_for_unobserved_indels
                    )
        ...
        self.scaled_expected_distribution_list = (
            self.__scale_expected_distribution_list()
        )
    ...
    def __calc_strand_enrichment(self):
        self.forward_enrichment_pval = self.__single_strand_binomial_test(
            StrandDirection.FORWARD
        )
        self.reverse_enrichment_pval = self.__single_strand_binomial_test(
            StrandDirection.REVERSE
        )

        if (
            self.forward_enrichment_pval is None
            and self.reverse_enrichment_pval is None
        ):
            self.strand_enrichment_pval = 1
            self.lesser_strand_enrichment_pval = 1
            ...
            self.strand_enrichment_pval = (
                self.forward_enrichment_pval * self.reverse_enrichment_pval
            )
            self.lesser_strand_enrichment_pval = max(
                self.forward_enrichment_pval, self.reverse_enrichment_pval
            )
    ...
    def __scale_expected_distribution_list(self):
        return scale_contingency_table(
            self.expected_distribution_list, self.num_of_observations_actual
        )
