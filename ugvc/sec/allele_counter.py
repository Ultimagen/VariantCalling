...
def count_alleles_in_pileup(pc: PileupColumn) -> Dict[str, ReadCounts]:
    ...
    for seq, quality in zip(
        pc.get_query_sequences(bool_add_indels=True), pc.get_query_qualities()
    ):
    ...
    * If no variant is called on this sample, we don't know the strand bias
    ...
    if "SB" in call:
        ...
        if is_multi_allelic:
            ...
            strand_bias = [
                int(sb) for sb in "|".join(variant.info["AS_SB_TABLE"]).split("|")
            ]
        ...
        allele_counts[ref] = ReadCounts(
            forward_support=strand_bias[0], reverse_support=strand_bias[1]
        )
