...
def pileup_to_efm(
    pc: PileupColumn, true_genotype: Tuple[str, str]
) -> ErrorFrequencyMatrix:
    matrix = np.zeros((6, 2))
...
def get_error_type(allele: str, genotype: Tuple[str, str]):
    ...
    if (
        len(allele) == 2
        and allele[0] == allele[1]
        and (genotype[0][-1] == allele[0] or genotype[1][-1] == allele[0])
    ):
        return 2
    # homopolymer insertion of 2 bases
    if (
        len(allele) == 3
        and allele[0] == allele[1] == allele[2]
        and (genotype[0][-1] == allele[0] or genotype[1][-1] == allele[0])
    ):
        return 3
    # base error mutation
