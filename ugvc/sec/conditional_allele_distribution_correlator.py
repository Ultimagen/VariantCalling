...
def correlate_sec_records(sec_records: List[SECRecord], min_scale: int = 50) -> float:
    ...
    for sec_record in sec_records:
        if (
            sec_record.were_observed_alleles_in_db
            and sec_record.likelihood > max_likelihood / 1000
        ):
            scale_to = min(sec_record.num_of_observations_expected, min_scale)
            scaled_distribution = scale_contingency_table(
                sec_record.expected_distribution_list, scale_to
            )
...
def correlate_distributions_per_pos(cad: ConditionalAlleleDistribution) -> float:
    ...
    for conditioned_genotype in cad.num_of_samples_with_alleles:
        all_possible_observed_alleles.update(
            cad.get_possible_observed_alleles(conditioned_genotype)
        )
    ...
    for observed_alleles in all_possible_observed_alleles:
        ...
        for conditioned_genotype in cad.num_of_samples_with_alleles:
            if (
                observed_alleles
                in cad.num_of_samples_with_alleles[conditioned_genotype]
            ):
        ...
        min_correlation = min(
            find_min_correlation(expected_distribution_lists), min_correlation
        )
