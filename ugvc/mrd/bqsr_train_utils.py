...
def prepare_featuremap_for_model(
    ...
    balanced_sampling_info_fields : list[str], optional
        Fields for balanced sampling, by default None
    ...
    random_seed : bool, optional
        Whether to use a random seed for the sampling of the input featuremap
...
class BQSRTrain:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        RuntimeError
            if model params are not available
...
def create_filters_file(filters_file_path):
    ...
    with open(filters_file_path, "w", encoding="utf8") as f:
