...
fields_to_collect = {
    "numeric_aggregated_fields": [FeatureMapFields.FILTERED_COUNT.value],
...
def write_a_pileup_record(
    ...
    record_dict[VAF] = record_dict[FeatureMapFields.FILTERED_COUNT.value] / record_dict[DP]
    record_dict[GT] = (0, 1)
    record_dict[AD] = [
        record_dict[DP] - record_dict[FeatureMapFields.FILTERED_COUNT.value],
        record_dict[FeatureMapFields.FILTERED_COUNT.value],
    ]
    ...
    if rec.qual < min_qual:
        curr_filter = "LowQual"
    elif rec.qual >= min_qual and record_dict[FeatureMapFields.FILTERED_COUNT.value] == 1:
        curr_filter = "SingleRead"
    else:
        curr_filter = "PASS"
...
def pileup_featuremap(
    ...
    # header.filters.add("PASS", None, None, "Aggregated quality above threshold, more than a single alternative read")
    header.filters.add(
    ...
    for field in fields_to_collect["numeric_aggregated_fields"]:
        header.info.add(
            field, orig_header.info[field].number, orig_header.info[field].type, orig_header.info[field].description
        )
    for field in fields_to_collect["numeric_array_fields"]:
    ...
    with pysam.VariantFile(featuremap) as f:
        ...
        for rec in f.fetch(chrom, start, end):
            ...
            if FeatureMapFields.FILTERED_COUNT.value not in cons_dict[rec_id]:
                ...
                for field in fields_to_collect["numeric_aggregated_fields"]:
                    cons_dict[rec_id][field] = 0
                # exceptions: X_QUAL
                cons_dict[rec_id]["X_QUAL"] = np.array([])
                for field in fields_to_collect["numeric_array_fields"]:
            ...
            # update the record
            for field in fields_to_collect["numeric_aggregated_fields"]:
                cons_dict[rec_id][field] += 1
            # exceptions: X_QUAL
            cons_dict[rec_id]["X_QUAL"] = np.append(cons_dict[rec_id]["X_QUAL"], rec.qual)
            for field in fields_to_collect["numeric_array_fields"]:
            ...
            for field in fields_to_collect["fields_to_write_once"]:
                cons_dict[rec_id][field] = rec.info[field]
            cons_dict[rec_id][DP] = rec.info[FeatureMapFields.READ_COUNT.value]
            prev_key = rec_id
        # write last record
        write_a_pileup_record(cons_dict[prev_key], prev_key, out_fh, header, min_qual)
