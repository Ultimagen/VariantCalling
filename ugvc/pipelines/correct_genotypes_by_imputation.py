...
def get_parser() -> argparse.ArgumentParser:
    ap_var = argparse.ArgumentParser(prog="correct_genotypes_by_imputation.py", description="Correct the genotypes in a vcf using imputation")
    ap_var.add_argument(
        "--input_vcf",
    ...
    ap_var.add_argument("--threads_for_contig", help="Number of threads to use when looping across contigs", type=int, default=4)
    ap_var.add_argument("--threads_beagle", help="Number of threads when running Beagle per contig", type=int, default=1)
    ap_var.add_argument(
        "--verbosity",
    ...
    ap_var.add_argument("--add_counting_category",
                        help="Is the input a result of mutect",
                        action="store_true")
    " - "
    return ap_var


def _command_to_subset_vcf(chrom, vcf, outfile):
    cmd = f'''bcftools view -O z -o {outfile} {vcf} {chrom} && \
    bcftools index -t {outfile}
    '''
    return cmd

def _command_to_filter_gq_vcf(vcf, outfile):
...
def _command_to_run_beagle(vcf, ref, gmap, outfile, nthreads=1):
...
def _command_to_filter_and_collpase_beagle(vcf, outfile):
...
def _command_to_annotate_with_beagle(vcf, beagle_collapsed_vcf, outfile):
    cmd = f'''bcftools annotate --annotations {beagle_collapsed_vcf} --columns INFO/IMP,INFO/DR2,FORMAT/DS -o {outfile} -O z  {vcf} && \
    bcftools index -t {outfile}
    '''
    return cmd
...
def _f_along_allele(allele_i, gt_ar, f_hom, f_het):
    ar = (np.any(gt_ar == allele_i, axis=1) & (gt_ar[:, 0] == gt_ar[:, 1])) * f_hom[allele_i - 1] + \
         (np.any(gt_ar == allele_i, axis=1) & (gt_ar[:, 0] != gt_ar[:, 1])) * f_het[allele_i - 1]
    return (ar)


def modify_stats_with_imp(pl_tup, num_alt, ds_ar, current_gt, epsilon):
    """
    ...
    f_hom = np.maximum(epsilon, np.minimum(np.maximum(ds_ar, 1) - 1, 1 - epsilon))
    # "Reshape" the f-values in the form of the gt_ar: For each allele, place the relevant value (het/hom) in the relevant position.
...
def add_imputation_to_vcf(beagle_anno_vcf, out_vcf, epsilon, region=None, add_counting_category=False):
    """
    ...
    counters = defaultdict(lambda: {"pass": 0,
                      "has_non_ref_imp": 0,
                      "imp_has_different_gt": 0,
                      "changed_gt": 0})

    logger.info("Modifying PLs and writing to " + out_vcf)
    with VariantFile(beagle_anno_vcf) as in_vcf_obj:
        ...
        in_vcf_obj.header.formats.add(id="GT0", number=gt_format.number,
                                       type=gt_format.type,
                                       description=gt_format.description + " (DeepVariant output)")

        gq_format = in_vcf_obj.header.formats["GQ"]
        in_vcf_obj.header.formats.add(id="GQ0", number=gq_format.number,
                                       type=gq_format.type,
                                       description=gq_format.description + " (DeepVariant output)")

        pl_format = in_vcf_obj.header.formats["PL"]
        in_vcf_obj.header.formats.add(id="PL0", number=pl_format.number,
                                       type=pl_format.type,
...
def run(argv: list[str]):
    ...
    with open(args.chrom_to_cohort_vcf, "r") as json_file:
        chrom_to_cohort = json.load(json_file)

    # Load the mapping between the chromosome names and the reference cohort files
    with open(args.chrom_to_plink, "r") as json_file:
    ...
    subset_vcf_cmds = [_command_to_subset_vcf(chrom, args.input_vcf, subset_files[chrom]) for chrom in chrom_to_cohort.keys()]
    ...
    high_gq_cmds = [_command_to_filter_gq_vcf(subset_files[chrom], high_gq_files[chrom]) for chrom in chrom_to_cohort.keys()]
    ...
    beagle_cmds = [_command_to_run_beagle(high_gq_files[chrom], chrom_to_cohort[chrom], chrom_to_plink[chrom], beagle_files[chrom], args.threads_beagle)
                   for chrom in chrom_to_cohort.keys()]
    sp.run_parallel(beagle_cmds, args.threads_for_contig)
    beagle_collapsed_files = {chrom: os.path.join(args.temp_dir, f'beagle_collapsed.{chrom}.vcf.gz') for chrom in chrom_to_cohort.keys()}
    beagle_collapse_cmds = [_command_to_filter_and_collpase_beagle(beagle_files[chrom], beagle_collapsed_files[chrom]) for chrom in chrom_to_cohort.keys()]
    sp.run_parallel(beagle_collapse_cmds, args.threads_for_contig)
    beagle_anno_files = {chrom: os.path.join(args.temp_dir, f'beagle_anno.{chrom}.vcf.gz') for chrom in chrom_to_cohort.keys()}
    beagle_anno_cmds = [_command_to_annotate_with_beagle(subset_files[chrom], beagle_collapsed_files[chrom], beagle_anno_files[chrom]) for chrom in chrom_to_cohort.keys()]
    sp.run_parallel(beagle_anno_cmds, args.threads_for_contig)

    def run_add_imputation_to_vcf(chrom):
        out = add_imputation_to_vcf(beagle_anno_vcf=beagle_anno_files[chrom],
                                    out_vcf=os.path.join(args.temp_dir, f'add_imp.{chrom}.vcf.gz'),
                                    epsilon=args.epsilon,
                                    add_counting_category=args.add_counting_category)
        return out

    counts_per_chrom = Parallel(n_jobs=args.threads_for_contig, verbose=10)(delayed(run_add_imputation_to_vcf)(chrom) for chrom in chrom_to_cohort.keys())
    ...
    logger.info("Writing statistics to " + args.stats_file)
    lines = dict()
    ...
    with open(untouched_contigs_bed, "w") as regions_file:
    ...
    sorted_add_imp_files = [add_imp_files[chrom] for chrom in sorted(chrom_to_cohort.keys(), key=lambda k: contigs_info[k]["id"])]
    sp.print_and_run(f"bcftools concat --naive -O z -o {args.output_vcf} {' '.join(sorted_add_imp_files)} {untouched_contigs_rehead_vcf}")
