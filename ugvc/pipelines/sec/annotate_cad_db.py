...
def annotate_cad_db(conditional_allele_distributions: ConditionalAlleleDistributions, min_gt_correlation: float):
    for chrom, distributions_per_pos in conditional_allele_distributions.distributions_per_chromosome.items():
...
def print_annotation(chrom: str, pos: int, conditional_allele_distribution: ConditionalAlleleDistribution,
                     annotation: str):
    ...
    for conditioned_genotype in conditional_allele_distribution.num_of_samples_with_alleles:
        for observed_alleles in conditional_allele_distribution.num_of_samples_with_alleles[conditioned_genotype]:
            allele_counts_str = conditional_allele_distribution.get_allele_counts_string(conditioned_genotype,
                                                                                         observed_alleles)
            count = conditional_allele_distribution.num_of_samples_with_alleles[conditioned_genotype][observed_alleles]
            print(f'{chrom}\t{pos - 1}\t{pos}\t{annotation}\t{conditioned_alleles}\t{conditioned_genotype}\t'
                  f'{observed_alleles}\t{count}\t{allele_counts_str}')


def is_hard_to_distinguish_noise_from_variants(cad: ConditionalAlleleDistribution) -> bool:
    ...
    for conditioned_genotype, distributions_given_genotype in cad.num_of_samples_with_alleles.items():
...
def is_missing_ground_truth_snps(cad: ConditionalAlleleDistribution) -> bool:
    for conditioned_genotype, distribution_given_genotype in cad.num_of_samples_with_alleles.items():
