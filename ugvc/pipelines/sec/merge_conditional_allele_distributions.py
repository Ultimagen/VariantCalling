...
def get_args(argv: List[str]):
    ...
    parser.add_argument('--output_prefix', help='prefix to pickle files (per chromosome) '
                                                'of serialized ConditionalAlleleDistribution dicts', required=True)
...
def merge_conditional_allele_distributions(argv: List[str]):
    ...
    for file_name in open(args.conditional_allele_distribution_files):
        ...
        for line in open(file_name):
            ...
            conditional_allele_distributions.add_counts(chrom, pos, ConditionalAlleleDistribution(ground_truth_alleles,
                                                                                                  true_genotype,
                                                                                                  observed_alleles,
                                                                                                  allele_counts_dict,
                                                                                                  int(n_samples)))
