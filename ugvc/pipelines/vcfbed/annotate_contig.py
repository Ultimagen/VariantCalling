...
def get_args(argv):
    ...
    parser.add_argument(
        "--contig",
        required=True,
        help="name of contig to process (this script analyses a contig at a time, use  VcfAnnotator.process_vcf to process multiple contigs)"
    )
    parser.add_argument(
        "--chunk_size",
        required=True,
        help="size of chunks to process in memory"
    )
...
def run(argv):
    ...
    VcfAnnotator.process_contig(vcf_in=args.vcf_in,
