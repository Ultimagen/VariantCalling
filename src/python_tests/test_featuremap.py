...
@pytest.fixture
def data():
    seq = pyfaidx.Fasta(pjoin(PYTHON_TESTS_PATH, COMMON, "sample.fasta"))["chr20"][
        :
    ].seq.upper()
    return seq
...
def test_collect_coverage_per_motif(generate_coverage_test_set):
    ...
    result = fm._collect_coverage_per_motif(sequence, coverage, 5, 1).set_index(
        "motif_5"
    )
...
def test_intersect_featuremap_with_signature(tmpdir):
    ...
    fm.intersect_featuremap_with_signature(
