...
import python.modules.db_access as db_access
import pandas as pd
from os.path import dirname
import re
import os

CLASS_PATH = "db_access"


def test_database_access_available():
    assert not db_access.DISABLE_PAPYRUS_ACCESS, "Papyrus command line access not available, need to install pymongo and define $PAPYRUS_ACCESS_STRING"


def test_fetch_from_database():
...
hardcoded_wfids = ['de06922f-07f8-4b51-843e-972308c81c6f',
...
def test_inputs_outputs_dataframe():
    r = re.compile(r'150450.*')
    docs = db_access.query_database(
        {"metadata.runId": r, 'inputs': {'$exists': True}})
