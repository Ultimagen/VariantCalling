...
error_model = pd.read_excel("/home/ilya/proj/VariantCalling/work/190623/error_model.xls", index_col=0)
...
def getQualityStringRead(read: str) -> str:
    ...
    qualities = ''.join([getQualityStringMer(key[i], base_order[i]) for i in range(len(key))])
    return qualities
...
def getQualityVector(read: str) -> list:
    ...
    base_order = (DEFAULT_FLOW_ORDER * len(key))[:len(key)]
    qualities = sum([getQualityVals(key[i], base_order[i]) for i in range(len(key))], [])
    return qualities
