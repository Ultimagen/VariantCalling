import pathmagic
import python.modules.filtering.blacklist as blacklist_fcn
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    ap.add_argument(
        "--input_file", help="Name of the input h5/vcf file. h5 is output of comparison", type=str)
    ap.add_argument(
        "--blacklist", help="blacklist file by which we decide variants as FP", type=str, required=False)
    ...
    ap.add_argument("--list_of_contigs_to_read", nargs='*',
                    help="List of contigs to read from the DF", default=[])
...
def main(argv: List[str]):
    ...
    logger = logging.getLogger(__name__ if __name__ !=
                           "__main__" else "train_model_pipeline")
    ...
    try:
        ...
        if with_dbsnp_bl:
            if args.list_of_contigs_to_read != []:
                dfs = [vcftools.get_vcf_df(args.input_file, chromosome=x)
        ...
        if args.mutect:
            df['qual'] = df['tlod'].apply(
                lambda x: max(x) if type(x) == tuple else 50)*10
        df.loc[
        ...
        for m in ['threshold', 'threshold_train', 'rf', 'rf_train']:
            ...
            prcdict[name_optimum] = results_dict[name_optimum.replace(
                "recall_precision", "recall_precision_curve")].set_index("group")
        ...
        if args.evaluate_concordance:
            ...
            calls_df, _ = vcf_pipeline_utils.annotate_concordance(calls_df, args.reference,
                                                                  ...
                                                                  annotate_intervals=args.annotate_intervals)
            if args.mutect:
                calls_df['qual'] = calls_df['tlod'].apply(
                    lambda x: max(x) if type(x) == tuple else 50) * 10
            calls_df.loc[
