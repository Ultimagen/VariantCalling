...
def parse_params_file(params_file, pipeline_name):
    ...
    ap.add('--em_vc_number_of_cpus', required=False,
           help="Number of CPUs on the machine", type=int, default=12)
    if pipeline_name == "error_metrics":
        ap.add('--em_vc_demux_file', help="Path to the demultiplexed bam")
        ap.add('--em_vc_number_to_sample', required=False,
               help="Number of records to downsample", type=int)
        ...
        ap.add('--rqc_chromosome', help="Single chromosome to filter for",
               required=True, type=str)
        ap.add('--rqc_evaluation_intervals_names', required=False,
               help="Comma separated list of evaluation interval names", type=str)
        ap.add('--rqc_evaluation_intervals', required=False,
               help="Comma separated list of evaluation interval file names", type=str)
        ap.add('--rqc_coverage_intervals_table', required=False,
               help="File with TSV coverage evaluation intervals", type=str)
    elif pipeline_name == "variant_calling":
        ap.add('--em_vc_demux_file', help="Path to the demultiplexed bam")
        ap.add('--em_vc_recalibration_model', required=False,
               help="recalibration model (h5)")
        ap.add('--em_vc_ground_truth', required=False,
               help="Ground truth file to compare", type=str)
        ap.add('--em_vc_ground_truth_highconf', required=False,
               help="Ground truth high confidence file", type=str)
        ap.add('--em_vc_gaps_hmers_filter', required=False,
               help="Bed file with regions to filter out", type=str)
        ap.add('--em_vc_chromosomes_list', required=False,
               help="File with the list of chromosomes to test")
...
def head_file(input_file, output_file, number_to_sample, nthreads):
    ...
    cmd1 = ["samtools", "view", '-@%d' %
            max(1, int((nthreads - 2) / 2)), '-h', input_file]

    cmd2 = ["head", f"-{number_to_sample+100}"]

    cmd3 = ["samtools", "view", '-@%d' %
            max(1, int((nthreads - 2) / 2)), "-b", "-o", output_bam, "-"]
    with open(output_err, 'w') as output_err_handle:
        ...
        task3 = subprocess.Popen(
            cmd3, stdin=task2.stdout, stderr=output_err_handle)
        task2.stdout.close()
...
def align(input_file, output_file, genome_file, nthreads):
    ...
    if type(input_file) == list:
        cmd1 = ['picard', '-Xms5000m', 'SamToFastq', 'INPUT=%s' %
                input_file[0], 'FASTQ=/dev/stdout']
    else:
        cmd1 = ['picard', '-Xms5000m', 'SamToFastq', 'INPUT=%s' %
                input_file, 'FASTQ=/dev/stdout']
    ...
    output_err_handle.write(" ".join(cmd2))
    output_err_handle.flush()
    task2 = subprocess.Popen(cmd2, stdin=task1.stdout,
    ...
    output_err_handle.write("\n")
    output_err_handle.flush()
    task3 = subprocess.Popen(cmd3, stdin=task2.stdout,
                             stdout=output_err_handle, stderr=output_err_handle)
    task2.stdout.close()
    output = task3.communicate()
...
def filter_quality(input_file, output_files, nthreads):
    ...
    cmd = ['samtools', 'view', '-q20', '-@%d' %
           max(1, (nthreads - 1)), '-b', '-o', output_bam, input_file]
    with open(output_err, 'w') as output_err_handle:
...
def sort_file(input_file, output_file, nthreads):
    output_bam, output_err = output_file
    with open(output_err, "w") as output_err_handle:
        cmd1 = ['samtools', 'sort', '-@%d' %
                max(1, (nthreads - 1)), '-T', dirname(output_bam), '-o', output_bam, input_file[0]]
...
def recalibrate_file(input_file, output_files, recalibration_model, nthreads):
    ...
    with open(output_err, 'w') as output_err_handle:
        ...
        subprocess.check_call(
            " ".join(cmd1), stdout=output_err_handle, stderr=output_err_handle, shell=True)
...
def collect_alnstats(idxstats_file: str, filter_metrics: str) -> pd.DataFrame:
    ...
    df1 = pd.read_csv(filter_metrics, sep="\t",
                      comment="#", engine="python").T[0]
    ...
    df.loc['aligned', '%'] = np.round(
        df.loc['aligned', 'Million reads'] / df.loc['total', 'Million reads'] * 100, 2)
    df.loc['unaligned', '%'] = np.round(
        df.loc['unaligned', 'Million reads'] / df.loc['total', 'Million reads'] * 100, 2)
    df.loc['hq aligned', '%'] = np.round(
        df.loc['hq aligned', 'Million reads'] / df.loc['total', 'Million reads'] * 100, 2)
        ...
        with open(input_file) as infile:
            for line in infile:
                if not line.startswith("[M::bam2fq_mainloop]"):
                    ...
                    matches = re.search(
                        r'\[M::bam2fq_mainloop\] processed ([0-9]*) reads', line)
