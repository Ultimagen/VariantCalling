...
def parse_params_file(params_file, pipeline_name):
    ...
    if pipeline_name == 'rapidqc':
        ...
        assert len(args.rqc_evaluation_intervals) == len(args.rqc_evaluation_intervals_names), 'Different length of names and evaluation intervals given'
...
def generate_rqc_output(dup_ratio: float, metrics: pd.DataFrame, histogram: pd.DataFrame, total_reads: int) -> tuple:
    ...
    histogram['cum_cov'] = histogram['high_quality_coverage_count'].cumsum() / histogram['high_quality_coverage_count'].cumsum().max()
    covs = histogram['coverage'].loc[np.searchsorted(histogram['cum_cov'], [0.05, 0.1, 0.2, 0.5])]
        ...
        with open(input_file) as infile:
            for line in infile:
                if not line.startswith('INFO'):
                    ...
                    matches = re.search('Wrote ([0-9]*) alignment records', line)
