...
with open(pjoin(params.em_vc_output_dir, logname),'w', buffering = 1) as output_log : 
    ...
    try: 
        ...
        vc_pipeline.run(multiprocess=params.em_vc_number_of_cpus, logger=logger, verbose=6)

        mark_duplicates_metrics_file = (mark_duplicates_bam._get_output_files(True, []))
        mark_duplicates_metrics_file = vc_pipeline_utils.flatten(mark_duplicates_metrics_file)
        mark_duplicates_metrics_file = [ x for x in mark_duplicates_metrics_file if x.endswith('rmdup.metrics')][0]
        ...
        cvg_metrics_files = vc_pipeline_utils.flatten([ (x._get_output_files(True, [])) for x in coverage_stats_tasks ])
        cvg_metrics_files = [ x for x in cvg_metrics_files if x.endswith('coverage.metrics')]        
