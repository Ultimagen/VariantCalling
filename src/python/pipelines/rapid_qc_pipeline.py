		...
		try:
				...
				if 'rqc_coverage_intervals_table' in vars(params).keys():
					...
					coverage_intervals_table['file'] = \
						coverage_intervals_table['file'].apply(lambda x: pjoin(params.rqc_coverage_intervals_location, x))
					...
					coverage_categories = vc_pipeline.product(vc_pipeline_utils.coverage_stats,
												sorted_bam, ruffus.formatter("sort.bam"),
												coverage_intervals_files, ruffus.formatter("interval_list"),
												[pjoin(params.em_vc_output_dir,"{basename[0][0]}.{basename[1][0]}.metrics"),
												pjoin(params.em_vc_output_dir,"logs", "{basename[0][0]}.{basename[1][0]}.coverage.log")],
												extras=[params.em_vc_genome, None],
												name="multiple_stats")
