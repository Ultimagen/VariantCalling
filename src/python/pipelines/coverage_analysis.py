...
def generate_stats_and_plots(
    ...
    df_coverage: str
        Path to pandas dataframe created by calculate_and_bin_coverage
    df_annotations: str
        Path to pandas dataframe created by create_coverage_annotations (with df_coverage as input)
    out_path: str
    ...
    if color_group is None:
        color_group = range(df_annotations.shape[1])
...
Numbers shown for median and 5th percentile""",
    ...
    if out_path is None:
        ...
        df_stats.to_hdf(coverage_stats_dataframes, key="stats", mode="w")
