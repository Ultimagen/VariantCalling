...
def find_thresholds(concordance: pd.DataFrame, classify_column: str = 'classify') -> pd.DataFrame:
    ...
    results[('recall', 'indel')] = results.get(('tp', True), 0) / \
        (results.get(('tp', True), 0) + results.get(('fn', True), 0) + 1)
    results[('specificity', 'indel')] = results.get(('tp', True), 0) / \
        (results.get(('tp', True), 0) + results.get(('fp', True), 0) + 1)
    results[('recall', 'snp')] = results.get(('tp', False), 0) / \
        (results.get(('tp', False), 0) + results.get(('fn', False), 0) + 1)
    results[('specificity', 'snp')] = results.get(('tp', False), 0) / \
        (results.get(('tp', False), 0) + results.get(('fp', False), 0) + 1)
    results.index = pairs
    return results
