...
if args.short_report_h5 is not None:
    with pd.HDFStore(args.short_report_h5) as hdf:
        hdf_keys=hfd.keys()
...
if args.extended_report_h5 is not None:
    with pd.HDFStore(args.extended_report_h5) as hdf:
        hdf_keys=hfd.keys()
        for report_key in hdf_keys:
            extended_report_h5_pd = pd.read_hdf(args.extended_report_h5,key= report_key)
            extended_report_h5.to_hdf(args.output_h5,key="short_report_" + key, mode="a")