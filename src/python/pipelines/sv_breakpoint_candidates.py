...
def SV_breakpoint_candidates(csv_filename, output_file_prefix):
    ...
    for ind in [f"chr{i}" for i in range(1,24)]: ## todo: add x,y?
        ...
        for chunk in chunks:
            chunk.columns = ['read_id', 'flag', 'chr', 'pos', 'mapQ', 'cigar', 'SA_info']
            mask = chunk['chr'] == ind
        ...
        SA_info = df['SA_info'].str.split(',', expand=True).rename(
            columns={0: 'SA_chr', 1: 'SA_chr_start', 2: 'SA_strand', 3: 'SA_cigar', 4: 'SA_MQ', 5: 'SA_unknown'})
        ...
        df['argmi'] = (
            np.argmin([df['start_softclip'], df['end_softclip'], df['SA_start_softclip'], df['SA_end_softclip']], axis=0))
        ...
        res = pd.concat(
            [df[['read_id', 'chr', 'junction_pos', 'SA_chr_id', 'SA_junction_pos', 'firstpart', 'mapQ', 'SA_MQ']],
             df['chr_end'] - df['pos'],
        ...
        final_res = gb.agg({'mapQ': 'mean', 'SA_MQ': 'mean', 'len': 'mean', 'SA_len': 'mean'})
        ...
        if not final_res_bed.empty:
            final_res_bed.to_csv(f"{file_basename}.bed", sep='\t', header=False, index=False)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output_file_prefix", help="Output file prefix",
                    type=str, required=True)
    args = ap.parse_args()

    SV_breakpoint_candidates(args.input_file, args.output_file_prefix)
