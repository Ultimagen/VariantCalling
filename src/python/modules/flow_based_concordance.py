...
def reinterpret_variants(concordance_df: pd.DataFrame, input: dict, fasta: pyfaidx.Fasta) -> pd.DataFrame:
...
def _convert_fns_to_tps(_df: pd.DataFrame) -> pd.DataFrame:
    ...
    subdf = _df.loc[change_annotation, ["indel", "classify",
                                        "hmer_indel_length", "qual", 'sor']].copy()
    subdf["classify"] = "tp"
    subdf['qual'] = 300
    subdf['sor'] = 1
