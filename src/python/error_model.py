...
def split_by_signal_bins( source_dataframe: pd.DataFrame) -> pd.DataFrame :
    ...
    Returns
    -------
    List (indexed by bin values) with columns of the form ?(n)
    '''
...
def convert2readGivenData( source_dataframe: pd.DataFrame) -> pd.DataFrame : 
    ...
    for err_idx in source_dataframe.columns[:bins_number] : 
        source_dataframe['dest({})'.format(err_idx)] = (source_dataframe.index.get_level_values('hmer_number')-err_idx).astype(np.int)
    ...
    for err_idx in source_dataframe.columns[:bins_number] : 
        ...
        result_dataframe.loc[dest_index,'P({})'.format(err_idx)] = np.array(tmp['P({})'.format(-err_idx)].values)[destination_contained]
...
class ErrorModel : 
    ...
    def get_hash( self, tuple_hash: int, bin: Optional[int]=None ) -> np.array : 
        if bin is None : 
            ...
            hashed_idx = self.hashed_dict[bin].get(tuple_hash, self.error_model[bin].shape[0]-1)
    ...
    def get_tuple( self, tup: tuple, bin: Optional[int]=None ) -> np.array : 
        if bin is None: 
            ...
            hashed_idx = self.hashed_dict[bin].get(hash(tup), self.error_model[bin].shape[0]-1)
            return self.error_model[bin][hashed_idx,:]
