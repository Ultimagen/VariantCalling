...
class TestConditionalAlleleDistribution(unittest.TestCase):
    def test_construction(self):
        ...
        self.assertEqual(
            10, cad.get_allele_count(ref_gt, "A,AG", "A", StrandDirection.FORWARD)
        )
        self.assertEqual(
            8, cad.get_allele_count(ref_gt, "A,AG", "A", StrandDirection.REVERSE)
        )
        self.assertEqual(
            1, cad.get_allele_count(ref_gt, "A,AG", "AG", StrandDirection.FORWARD)
        )
        self.assertEqual(
            0, cad.get_allele_count(ref_gt, "A,AG", "AG", StrandDirection.REVERSE)
        )
        self.assertEqual(
            "A 10,8,0 AG 1,0,0", cad.get_allele_counts_string(ref_gt, "A,AG")
        )
    ...
    def test_merging_two_records_with_same_alleles(self):
        cad = ConditionalAlleleDistribution(
            "A,T", ref_gt, "A,T", {"A": ReadCounts(10, 8), "T": ReadCounts(1, 0)}
        )
    ...
    def test_merging_two_records_with_different_alleles(self):
        cad1 = ConditionalAlleleDistribution(
            "A,T", ref_gt, "A,T", {"A": ReadCounts(10, 8), "T": ReadCounts(1, 0)}
        )
        cad2 = ConditionalAlleleDistribution(
            "A,T", ref_gt, "A,T", {"A": ReadCounts(8, 12), "G": ReadCounts(0, 1)}
        )
    ...
    def test_merging_two_record_with_unknown_strand_direction(self):
        cad1 = ConditionalAlleleDistribution(
            "A,T", ref_gt, "A,T", {"A": ReadCounts(10, 8), "T": ReadCounts(1)}
        )
        cad2 = ConditionalAlleleDistribution(
            "A,T", ref_gt, "A,T", {"A": ReadCounts(0, 0, 20)}
        )
        cad1.update_distribution(cad2)
        self.assertEqual(
            ReadCounts(10, 8, 20), cad1.get_allele_counts(ref_gt, "A,T", "A")
        )
        self.assertEqual(ReadCounts(1, 0), cad1.get_allele_counts(ref_gt, "A,T", "T"))
    ...
    def test_merging_two_records_with_different_genotype(self):
        ...
        """
        cad1 = ConditionalAlleleDistribution(
            "A,T", "0/0", "A,T", {"A": ReadCounts(10, 8), "T": ReadCounts(1, 0)}
        )
        cad2 = ConditionalAlleleDistribution(
            "A,T", "1/1", "A,T", {"A": ReadCounts(), "T": ReadCounts(11, 12)}
        )
        cad1.update_distribution(cad2)
