...
from ugvc.utils.stats_utils import (
    get_corrected_multinomial_frequencies,
    get_f1,
    get_precision,
    get_recall,
...
class TestStatsUtils(unittest.TestCase):
    ...
    def test_get_corrected_multinomial_frequencies(self):
        np.testing.assert_array_equal(
            np.array([1, 1, 1]) / 3, get_corrected_multinomial_frequencies([10, 10, 10])
        )
        np.testing.assert_array_equal(
            np.array([11, 11, 1]) / 23,
    ...
    def test_multinomial_likelihood(self):
        self.assertAlmostEqual(
            0.0652, multinomial_likelihood([4, 4, 4], [4, 4, 4]), places=3
        )
        self.assertAlmostEqual(
            0.0652, multinomial_likelihood([4, 4, 4], [40, 40, 40]), places=3
        )
        self.assertAlmostEqual(
            0.0068, multinomial_likelihood([40, 40, 40], [40, 40, 40]), places=3
        )
        ...
        self.assertAlmostEqual(
            0.039, multinomial_likelihood([1, 10, 40], [1, 10, 40]), places=3
        )

        # 0 expected does not cause 0 likelihood, because of add-one correction
        self.assertAlmostEqual(
            0.0043, multinomial_likelihood([4, 4, 4], [4, 4, 0]), places=3
        )
    ...
    def test_multinomial_likelihood_ratio(self):
        self.assertAlmostEqual(
            1, multinomial_likelihood_ratio([4, 4, 4], [4, 4, 4])[1], places=3
        )
        self.assertAlmostEqual(
            1, multinomial_likelihood_ratio([4, 4, 4], [40, 40, 40])[1], places=3
        )
        self.assertAlmostEqual(
            1, multinomial_likelihood_ratio([40, 40, 40], [40, 40, 40])[1], places=3
        )
        ...
        self.assertAlmostEqual(
            1, multinomial_likelihood_ratio([1, 10, 40], [1, 10, 40])[1], places=3
        )

        # 0 expected does not cause 0 likelihood, because of add-one correction
        self.assertAlmostEqual(
            0.0661, multinomial_likelihood_ratio([4, 4, 4], [4, 4, 0])[1], places=3
        )
