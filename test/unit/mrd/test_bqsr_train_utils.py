...
def test_prepare_featuremap_for_training(tmpdir):
    ...
    expected_training_featuremap_vcf = pjoin(
        inputs_dir, "333_CRCs_39_LAv5and6.featuremap.single_substitutions.subsample.intersect.test.downsampled.2.vcf.gz"
    )
    downsampled_training_featuremap_vcf, downsampled_test_featuremap_vcf = prepare_featuremap_for_model(
        workdir=tmpdir, input_featuremap_vcf=input_featuremap_vcf, training_set_size=12, test_set_size=3
    )
    assert filecmp.cmp(downsampled_training_featuremap_vcf, expected_training_featuremap_vcf)
    assert filecmp.cmp(downsampled_test_featuremap_vcf, expected_training_featuremap_vcf)
