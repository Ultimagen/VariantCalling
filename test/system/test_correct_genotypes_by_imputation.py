...
class TestCorrectGenotypesByImputation:
    ...
    test_inputs_dir = f"{test_dir}/resources/system/test_correct_genotypes_by_imputation"

    def test_correct_genotypes_by_imputation(self, tmpdir):
        os.makedirs(tmpdir, exist_ok=True)
        with open(os.path.join(tmpdir, "chrom_to_cohort_vcf.json"), "w") as f:
            json.dump({"chr1": os.path.join(self.test_inputs_dir, "1kGP_phased_panel.chr1.vcf.gz"),
                       "chr2": os.path.join(self.test_inputs_dir, "1kGP_phased_panel.chr2.vcf.gz")},
                      f)
        with open(os.path.join(tmpdir, "chrom_to_plink.json"), "w") as f:
            json.dump({"chr1": os.path.join(self.test_inputs_dir, "plink.chr1.GRCh38.chr.map"),
                       "chr2": os.path.join(self.test_inputs_dir, "plink.chr2.GRCh38.chr.map")},
                      f)
        input_vcf = os.path.join(self.test_inputs_dir, "dv_013783-X0013.vcf.gz")
        ...
        df0 = pd.read_csv(os.path.join(self.test_inputs_dir, "out_counts.csv"), index_col=0)
